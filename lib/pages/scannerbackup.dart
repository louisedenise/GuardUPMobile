...
class _QrCodeScannerState extends State<QrCodeScanner> {
  ...
  Barcode? result;
  bool isScanned = false; // Flag to prevent duplicate entries
  DateTime? lastScanTime; // Timestamp of the last scan
  late String lastScannedBuildingCode; // Declare this variable at the top of the function or in the enclosing scope.
...
Future<void> saveScannedData(String buildingCode) async {
  ...
  if (currentUser != null) {
    ...
    if (latestQuery.docs.isNotEmpty) {
      ...
      if (latestBuildingCode == buildingCode) {
        ...
        if (exitTimestamp == '~') {
          ...
          // No need to create a new entry since the exitTimestamp was updated
          return;
        } else {
          // If exitTimestamp is not empty, show a message
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
    ...
    final currentEntry = {
      ...
      'entryTime': timeFormat.format(currentTime), // Format and save as string
      'exitTimestamp': '~', // Empty initially or set a default value if desired
      'userEmail': currentUserEmail,
    };

    // Save the current entry to Firestore
